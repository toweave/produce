...
export function VideoPlayer({ videoRef }: VideoPlayerProps): React.JSX.Element {
  ...
  const pollError = useSeedance2CreateStore((s) => s.pollError)
  const createdId = useSeedance2CreateStore((s) => s.createdId)
  const taskStatus = useSeedance2CreateStore((s) => s.taskStatus)
  const isPlaying = useSeedance2CreateStore((s) => s.isPlaying)
  const currentTime = useSeedance2CreateStore((s) => s.currentTime)
  const captureFlash = useSeedance2CreateStore((s) => s.captureFlash)
  ...
  const handlePlayPause = (): void => {
    ...
    if (!video) return
    const store = useSeedance2CreateStore.getState()
    if (video.paused) {
  ...
  const handleTimeUpdate = (): void => {
    const video = videoRef.current
    if (video) useSeedance2CreateStore.getState().update({ currentTime: video.currentTime })
  }

  const handlePause = (): void => {
    useSeedance2CreateStore.getState().update({ isPlaying: false, hasInteracted: true })
  }

  const handlePlay = (): void => {
    useSeedance2CreateStore.getState().update({ isPlaying: true })
  }
  ...
  const handleCaptureKeyframe = (): void => {
    ...
    try {
      ...
      useSeedance2CreateStore.getState().update({ captureFlash: true })
      setTimeout(() => useSeedance2CreateStore.getState().update({ captureFlash: false }), 300)
    } catch { /* capture failed */ }
  }

  return (
    <div className="rounded-lg border border-border bg-card overflow-hidden flex flex-col relative min-h-[240px]">
      {!videoUrl && !pollError && (
      ...
      {pollError && !videoUrl && (
        <div className="flex flex-col items-center justify-center gap-3">
          ...
          <button
            onClick={() => useSeedance2CreateStore.getState().resetPanel()}
      ...
      {videoUrl && (
        <div
          className="relative bg-black flex items-center justify-center w-full max-w-[1280px] mx-auto"
        ...
        >
          ...
          <video
            ref={videoRef}
            src={videoUrl}
            className="w-full max-w-[1280px] h-auto max-h-[720px] object-contain cursor-pointer"
