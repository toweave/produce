    ...
    <div className="flex-1 rounded-lg border border-border bg-card overflow-hidden flex flex-col relative min-h-[240px]">
          ...
          {!createdId ? (
            ...
            <>
              ...
              <span className="text-xs text-muted-foreground">
                状态: {taskStatus === 'queued' ? '排队中' : taskStatus === 'running' ? '生成中' : taskStatus}
              </span>
            </>
          )}
        </div>
      ...
      {pollError && !videoUrl && (
        <div className="flex-1 flex flex-col items-center justify-center gap-3">
          <div className="rounded-md bg-destructive/10 p-4 text-sm text-destructive">{pollError}</div>
          <button onClick={onReset} className="text-sm text-primary hover:underline">重新开始</button>
        </div>
      )}
