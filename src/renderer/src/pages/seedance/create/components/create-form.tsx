...
export function CreateForm({ onSubmit }: CreateFormProps): React.JSX.Element {
  ...
  const {
    prompt, imageData, useLastFrame, lastFrameData,
    ...
    <div className="flex flex-col gap-4 overflow-y-auto">
          ...
          <FieldContent>
            <div className="flex items-center gap-2 mb-2">
              <Checkbox
                ...
                onCheckedChange={(checked) => update({ useLastFrame: !!checked })}
              />
      ...
      <div className="grid grid-cols-2 gap-4">
        <Field>
          ...
          <FieldContent>
            <Select value={ratio} onValueChange={(v) => update({ ratio: v as typeof ratio })}>
        ...
        <Field>
          ...
          <FieldContent>
            <Select value={String(duration)} onValueChange={(v) => update({ duration: Number(v) })}>
              <SelectTrigger className="w-full">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
      ...
      <FieldGroup>
        <Field orientation="horizontal" className="gap-2">
          <Checkbox
            id="watermark"
            checked={watermark}
            onCheckedChange={(checked) => update({ watermark: !!checked })}
          />
